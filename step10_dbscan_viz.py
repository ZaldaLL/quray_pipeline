#!/usr/bin/env python3
"""
Step 10 — DBSCAN plane segmentation visualization.

Offline visualisation output:
  - output/visualization/step10_dbscan_planes/
    - all_planes_colored.ply       ← main file (all planes coloured by ID, hsv colormap)
    - planes/plane_001.ply ...      (individual planes)
    - all_planes_colored_meta.json
  - ply_out/   (PLY copies)
"""

from __future__ import annotations

import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))

import matplotlib.pyplot as plt
import numpy as np

from quray_pipeline.config import OUTPUT_DIR
from quray_pipeline.utils.io_utils import load_npz
from quray_pipeline.utils.plotting import save_figure, setup_matplotlib
from quray_pipeline.utils.visualization import VisualizationManager


def _plot_dbscan_planes(data: np.ndarray, fig_num: int, title: str, annotate: bool = False) -> None:
    X = data[:, 0:3]
    labels = data[:, 7].astype(int)
    valid = labels != -1
    Xv = X[valid]
    labelsv = labels[valid]
    unique_labels = np.unique(labelsv)
    n_clusters = len(unique_labels)
    colors = plt.cm.hsv(np.linspace(0, 1, max(n_clusters, 1)))

    setup_matplotlib()
    fig = plt.figure(figsize=(14, 9), facecolor="w")
    ax = fig.add_subplot(111, projection="3d")

    color_idx = 0
    for plane_id in unique_labels:
        idx = labelsv == plane_id
        if np.sum(idx) < 50:
            continue
        ax.scatter(
            Xv[idx, 0],
            Xv[idx, 1],
            Xv[idx, 2],
            s=10,
            c=[colors[color_idx % len(colors)]],
            edgecolors="none",
        )
        if annotate:
            center = Xv[idx].mean(axis=0)
            ax.text(
                center[0],
                center[1],
                center[2] + 12,
                str(int(plane_id)),
                fontsize=10,
                fontweight="bold",
                color="black",
                ha="center",
                va="center",
            )
        color_idx += 1

    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_zlabel("Z (m)")
    ax.set_title(title)
    ax.grid(True)
    save_figure(fig_num, fig, matplotlib_show=False)
    plt.close(fig)


def main() -> None:
    data = load_npz(OUTPUT_DIR / "step09_all_planes.npz")
    all_planes = data["all_planes"]
    if all_planes.size == 0:
        print("No planes to visualize.")
        return

    # ---- Original matplotlib figure ----
    _plot_dbscan_planes(
        all_planes,
        fig_num=100,
        title="DBSCAN plane segmentation (random colours + plane ID labels)",
        annotate=False,
    )

    # ---- Unified offline visualisation (PLY + JSON + NPZ) ----
    vm = VisualizationManager("step10_dbscan_planes")
    vm.save_planes_colored(
        all_planes,
        name="all_planes",
        by="plane",
        min_points=50,
        export_per_plane=True,
        metadata={
            "step": 10,
            "description": "DBSCAN plane segmentation",
            "columns": ["x", "y", "z", "nx", "ny", "nz", "set_id", "plane_id"],
        },
    )

    # Save full raw data NPZ (for exact reconstruction in local viewer)
    X_raw = all_planes[:, 0:3]
    labels_raw = all_planes[:, 7].astype(int)
    valid = labels_raw != -1
    plane_ids = np.unique(labels_raw[valid])
    vm.save_npz(
        "all_planes_raw",
        points=X_raw,
        normals=all_planes[:, 3:6],
        set_ids=all_planes[:, 6].astype(int),
        plane_ids=all_planes[:, 7].astype(int),
    )

    # Save colormap mapping
    from quray_pipeline.utils.visualization import get_colormap_mapping
    cmap_mapping = get_colormap_mapping(labels_raw[valid], style="hsv")
    vm.save_json("all_planes_colormap", {
        "colormap": "hsv",
        "mapping": cmap_mapping,
        "n_planes": int(len(plane_ids)),
        "plane_ids": plane_ids.tolist(),
    })


if __name__ == "__main__":
    main()
